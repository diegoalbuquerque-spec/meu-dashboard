...
# So entram solicitacoes a partir desta data (a base com status comeca em set/2026).
DESDE = os.environ.get("DATA_INICIAL", "2026-09-01")
...
def main():
    ...
    dmin, dmax = regs[0]["data"], regs[-1]["data"]

    def ajusta_input(m):
        tag = m.group(0)
        valor = dmin if 'id="f-from"' in tag else dmax
        tag = re.sub(r'min="[^"]*"', 'min="%s"' % dmin, tag)
        tag = re.sub(r'max="[^"]*"', 'max="%s"' % dmax, tag)
        tag = re.sub(r'value="[^"]*"', 'value="%s"' % valor, tag)
        return tag

    html = re.sub(r'<input[^>]*id="f-(?:from|to)"[^>]*>', ajusta_input, html)
    html = re.sub(r'(DEFAULTS\s*=\s*\{\s*from:\s*")[^"]*(",\s*to:\s*")[^"]*("\s*\})',
                  lambda m: m.group(1) + dmin + m.group(2) + dmax + m.group(3), html, count=1)

    # O ?v= muda a cada carga para o navegador nao servir CSS antigo.
    tag = '<link rel="stylesheet" href="ajustes.css?v=%s">' % agora.strftime("%Y%m%d%H%M")
    if "ajustes.css" in html:
        html = re.sub(r'<link[^>]*href="ajustes\.css[^"]*"[^>]*>', tag, html, count=1)
    else:
